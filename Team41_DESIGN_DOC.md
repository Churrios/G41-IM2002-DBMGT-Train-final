<!-- 此檔為 design-document/design-document.md 之繳交複本（圖片連結已改寫為根目錄相對路徑）；內容修改請以該檔為源頭並同步本檔 -->
...
![ER Diagram](design-document/er-diagram.png)

[ER Diagram（PDF 版）](design-document/er-diagram.pdf)
...
## 範例二 — 端到端測試：分離「程式正確性」與「LLM 行為」

**背景（Context）：**
透過 chatbot 做端到端測試時出現多個錯誤回答——助理宣稱某站「沒有連線」（C6）、甚至回報一筆資料庫裡不存在的訂位「已確認」。我們無法分辨是資料庫查詢函式有 bug 還是 LLM 層出錯，而修錯層會浪費繳交前所剩的時間。

**提問（Prompt）：**
「我們的 chatbot 回答錯誤，但不確定 bug 在資料庫查詢函式還是 LLM 層。請設計一個能分離『程式正確性』與『LLM 行為』的測試策略，並產生一套直接呼叫的測試套件，涵蓋所有 graph / relational / Task 6 函式，且不能弄髒 seed 資料。」

**結果（Outcome）：**
AI 提出雙軌方法：49 項斷言的直接呼叫煙霧測試對真實資料庫逐函式驗證，與腳本化的 chatbot 測試並行，且 chatbot 的每個宣稱都用 SQL 直查資料庫複核。直接呼叫全數通過、chatbot 在相同情境卻失敗，證明缺陷在 LLM 側，由此導出三個精準修正（PR #59）：`query_station_connections` 回傳包成 `{station_id, connections}` envelope（LLM 不再誤判鄰站歸屬）；`report_delay` 在寫入前正規化 LLM 傳入的 severity（"serious"→high，否則違反 CHECK 約束）；並證實單輪 agent 無法串接 `get_available_seats`→`make_booking`（「任意座位」訂票幻覺——LLM 編造的 booking ID 經資料庫直查確認不存在）。AI 產生的測試套件本身也需要一次修正才可信：在 Windows cp950 主控台印 emoji 會崩潰，以 `sys.stdout.reconfigure(encoding="utf-8")` 修復。
...
**結果（Outcome）：**
AI 建議整份文件 embed：(1) 政策文件是獨立完整的單元——切 chunk 會將條件與定義分開；(2) nomic-embed-text 能良好處理段落長度的輸入。我們評估後**沒有完全採用**此建議：考量最長的政策文件超過 2000 字、整份 embed 成單一向量會稀釋語意焦點，最終 `seed_vectors.py` 改採 `chunk_text()` 切塊（每塊 300 字元、相鄰塊重疊 50 字元），每個 chunk 獨立 embed 儲存（seed 後 `policy_documents` 共 101 個 chunks）。其中「重疊 50 字元」的設計正是用來緩解 AI 指出的風險——條件與定義在 chunk 邊界被切開時，重疊區段仍保留上下文。於 live testing 以 RAG 政策問答驗證檢索正確：R1（誤點退費政策）、R2（捷運單車政策）均通過 ✅，回答內容均來自檢索到的文件段落而非模型自行生成。
...
取捨點在於：本地推論保護使用者隱私且消除 API 成本，但 `llama3.2:1b`（10 億參數）缺乏從 16 個工具函式中可靠選擇的指令跟隨能力。雙 provider 端到端實測（同一套 chatbot 測試腳本跑兩個 provider）中，本地模型頻繁選錯工具、甚至直接跳過工具呼叫（例如單車政策問題不走 `search_policy` 而自行作答），`gemini-3.1-flash-lite` 則全數選對；直接呼叫測試證實資料庫函式在兩個 provider 下行為完全一致，差異被隔離在 LLM 層。Gemini 需要網路連線與 API 憑證。
...
3. **面向 LLM 呼叫端的防禦性輸入驗證**：LLM agent 不像型別嚴格的應用程式客戶端——它傳入的是從自然語言推斷出的值。端到端實測抓到三個真實案例：`stops_travelled` 傳成字串 `"4"`（票價運算 crash，補 `int()` 轉型修復）；`severity` 傳 `"serious"`（違反 `delay_events` 的 CHECK 約束，後在 agent 層將同義詞正規化為 low/medium/high）；`hops=99`（觸發全圖遍歷逾時，上限 cap 在 10）。生產系統中，所有暴露給 LLM agent 的資料庫函式都需要在 dispatch 邊界先做驗證／正規化——DB 約束應是最後防線，而非第一道。

---

# Section 7 — Task 6 Extension: Delay Event Logging

> 負責人：三人共同 (文件由 蔣耀德 撰寫)

## 7.1 Motivation (動機)

在真實的軌道運輸網路中，突發的誤點與停駛事件是家常便飯。我們現有的 Graph 資料庫已經實作了強大的**誤點漣漪分析 (`query_delay_ripple` / C5)**，能夠算出某站誤點後影響的 N 跳車站。然而，系統原本缺乏一個能夠「即時寫入」與「查詢」誤點狀態的機制。

為了讓系統具備完整的真實世界運作能力，我們實作了「Delay Event Logging」功能：
1. 使用者可透過 Chatbot 直接回報誤點（Report Delay）。
2. 使用者可查詢目前活躍的誤點事件（Get Active Delays）。
這不僅使關聯式資料庫（PostgreSQL）多了即時寫入的情境，未來更能與 Graph DB 的 `delay_ripple` 查詢自然串聯，打造出從通報到影響分析的完整自動化流程。

## 7.2 Schema Snippet (表格設計)

我們在 `databases/relational/schema.sql` 中新增了 `delay_events` 資料表。
...
**設計決策**：`station_id` 刻意**不設置** Foreign Key (FK)。因為誤點事件可能發生在捷運站 (`metro_stations`) 或國鐵站 (`national_rail_stations`)。PostgreSQL 不支援單一欄位同時參照兩張不同父表的 FK。我們選擇在應用程式層確保資料的參照完整性，並加上適當的 Check Constraint 與 Index 來優化 `resolved_at IS NULL` 的「活躍誤點」查詢效能。

## 7.3 Example Queries (查詢範例與預期輸出)

**插入誤點事件 (INSERT)**
```python
# 呼叫 log_delay_event('MS03', 'high', 'Signal failure at platform 2')
```
對應的 SQL:
```sql
INSERT INTO delay_events (station_id, severity, description)
VALUES ('MS03', 'high', 'Signal failure at platform 2')
RETURNING event_id;
```
*預期輸出*：回傳新建事件的 `event_id` (例如 `1`)。

**查詢活躍誤點 (SELECT)**
```python
# 呼叫 get_active_delays('MS03')
```
對應的 SQL:
```sql
SELECT event_id, station_id, reported_at, severity, description
FROM delay_events
WHERE resolved_at IS NULL AND station_id = 'MS03'
...
[
  {
    "event_id": 1,
    "station_id": "MS03",
    "reported_at": "2026-06-09T10:00:00Z",
    "severity": "high",
    "description": "Signal failure at platform 2"
  }
]
```

## 7.4 Testing Evidence (測試截圖)

> 啟動 Docker、執行 seed 後，於 chatbot 實際輸入問題測試 `report_delay` 與 `get_active_delays` 兩個 tool。
> 環境：LLM Provider = Gemini (gemini-3.1-flash-lite)；**debug panel 開啟**，以佐證實際的工具呼叫與資料庫回傳（非 LLM 自行編造）。

### T1 — `report_delay`：回報新誤點

問句：`Report a high severity signal failure at Old Town MS07`
debug panel 顯示 `Calling: report_delay({station_id: MS07, severity: high, description: signal failure})`，回傳 `{event_id: 8, status: recorded}`，確認誤點已寫入 `delay_events` 表。

![T1 — report_delay 回報誤點](design-document/live-demo/T1.png)

### T2 — `get_active_delays`：查詢所有目前活躍的誤點

問句：`Are there any active delays right now?`
debug panel 顯示 `Calling: get_active_delays({})`，raw result 列出所有未解決（`resolved_at IS NULL`）的誤點，包含 T1 剛回報的 event 8，證明寫入與查詢串接正確。

![T2 — get_active_delays 查詢所有活躍誤點](design-document/live-demo/T2.png)

### T3 — `get_active_delays(station_id)`：依站別過濾

問句：`Any disruptions at MS07?`
debug panel 顯示 `Calling: get_active_delays({station_id: MS07})`，只回傳 MS07 的誤點（過濾掉其他站），證明站別過濾參數有效。

![T3 — get_active_delays 站別過濾](design-document/live-demo/T3.png)
