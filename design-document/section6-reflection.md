...
The trade-off: local inference preserves user privacy and eliminates API cost, but `llama3.2:1b` (1 billion parameters) lacks the instruction-following capacity needed to reliably select from 16 tool functions given ambiguous natural-language input. In dual-provider end-to-end testing (the same scripted chatbot scenarios run on both providers), the local model frequently called the wrong tool or skipped the tool call entirely — for example, answering a bicycle-policy question from its own parameters instead of calling `search_policy` — while `gemini-3.1-flash-lite` selected the correct tool in every scenario. Direct-call tests confirmed the database functions themselves behaved identically under both providers, isolating the difference to the LLM layer.
...
3. **Defensive input validation for an LLM caller**: Unlike a typed application client, an LLM agent passes whatever values it inferred from natural language. Our end-to-end testing caught three real cases: `stops_travelled` arriving as the string `"4"` (crashing the fare arithmetic until an `int()` cast was added), `severity` arriving as `"serious"` (violating the `delay_events` CHECK constraint until the agent layer normalised synonyms to low/medium/high), and `hops=99` (triggering a full-graph traversal timeout until capped at 10). In production, every database function exposed to an LLM agent needs a validation/normalisation layer at the dispatch boundary — DB constraints should be the last line of defence, not the first.
