...
| **Vector / RAG pipeline** — `seed_vectors.py`, `rag.py`, `reranker.py` *(no official task number; extends the pre-built skeleton)* | 蔣耀德 | — | Added `rag.py` retrieval module and `reranker.py` cross-encoder reranking stage (new files beyond the skeleton); made policy-document seeding idempotent; tuned `VECTOR_SIMILARITY_THRESHOLD` 0.3 → 0.5 after empirical false-positive testing; added guard to the agent fallback rule |
