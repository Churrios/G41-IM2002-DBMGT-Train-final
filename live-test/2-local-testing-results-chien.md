...
> 逐步測試流程與環境踩坑見 [local-testing-notes-chien.md](2-local-testing-notes-chien.md)。對照基準：[live-testing-results.md](1-live-testing-results.md)（蔡 06-05, Ollama）。
...
> 6 支 graph 函式均為 Chien 主責。直接呼叫結果全部正確（見 [local-testing-notes-chien.md](2-local-testing-notes-chien.md) §4）。
...
3. **Chien_本地測試指南.md（個人資料夾，未入庫）**：新增 Docker/Gemini 章節；修正 C3 測試問句（NR01→NR05 避 NR03 → MS01→MS09 避 MS07）。
